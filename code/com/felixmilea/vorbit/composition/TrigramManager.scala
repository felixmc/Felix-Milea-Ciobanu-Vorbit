...
class TrigramManager(dataSet: String) {
  private[this] val table = s"mdt_${dataSet}_c3"
  private[this] val db = new DBConnection(true)
  private[this] val query = db.conn.prepareStatement(s"SELECT * FROM `$table` ORDER BY `ngram1`, `ngram2`")

  private[this] val data: Map[(Int, Int), Map[Int, Int]] = {
    // get rows
    val rowSet = query.executeQuery()

    // builds final immutable hashmap to be accessed through API
    val dataBuilder = new MapBuilder[(Int, Int), Map[Int, Int], HashMap[(Int, Int), HashMap[Int, Int]]](new HashMap[(Int, Int), HashMap[Int, Int]])
    ...
    while (rowSet.next()) {
      val key = (rowSet.getInt("ngram1"), rowSet.getInt("ngram2"))
      val ngram3 = rowSet.getInt("ngram3")
  ...
  private val totalsCache: scala.collection.mutable.Map[(Int, Int), Int] = new scala.collection.mutable.HashMap[(Int, Int), Int]

  // gets total freqs for a single ngram1 or calculates it on spot and caches it
  def getTotal(id: (Int, Int)): Int = {
  ...
  def getNgram1(id: (Int, Int)): Map[Int, Int] = data.getOrElse(id, noFreqs)

  // gets the ngram2 from the bigram associated with the given freq for the given ngram1
  def getNgram2(id: (Int, Int), freq: Int): Int = {
    var tFreq = 0
  ...
  def getEndStates(id: (Int, Int)): Vector[Int] = data.get(id) match {
    case Some(state) => state.keys.toVector
    case None => Vector[Int]()
  }

  def getFreq(key: (Int, Int))(ngram2Id: Int): Int = getNgram1(key).getOrElse(ngram2Id, 0)
}